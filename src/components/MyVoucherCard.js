    ...
    <div className="min-h-[393px] bg-white rounded-lg shadow-md overflow-hidden relative">
      ...
      <div className="p-4">
          ...
          {voucher.tags.map((tag, index) => (
            <span
              ...
              className={`text-xs px-2 py-1 rounded-full ${
                tag === 'Hết hạn' ? 'bg-red-600 text-white' : 'bg-blue-100 text-blue-600'
              }`}
            >
              {tag}
            </span>
        ...
        <div className="flex justify-between mt-4 ">
          ...
          <button className="text-blue-600 border border-blue-600 px-3 py-1 rounded-full absolute bottom-5 right-5">
            Sử dụng voucher
          </button>
        </div>
      </div>
